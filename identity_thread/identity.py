...
def sync_for_stimuli(stimuli_type: str = "realtime") -> dict:
    ...
    Args:
        stimuli_type: "realtime" (fast, minimal) or "analytical" (deep, full)
    
    Returns:
        Synced identity data
    """
    # Determine context level based on stimuli type
    context_level = 1 if stimuli_type == "realtime" else 2
    
    # Sync and push with appropriate context level
    return push_identity(context_level=context_level)
