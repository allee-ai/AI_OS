...
import './StimuliPage.css';

interface StimuliSource {
  name: string;
  type: string;
  enabled: boolean;
...
export const StimuliPage = () => {
  const [sources, setSources] = useState<StimuliSource[]>([]);
  ...
  const fetchSources = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/stimuli/sources`);
  ...
  const fetchTemplates = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/stimuli/templates`);
      const data = await res.json();
      setTemplates(data);
    } catch (err) {
  ...
  const fetchSourceConfig = useCallback(async (name: string) => {
    ...
    setTestResult(null);
    try {
      const res = await fetch(`${API_BASE}/api/stimuli/sources/${name}`);
      const data = await res.json();
  ...
  const handleToggle = async (name: string) => {
    try {
      await fetch(`${API_BASE}/api/stimuli/sources/${name}/toggle`, { method: 'POST' });
      fetchSources();
  ...
  const handleTest = async (name: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/stimuli/sources/${name}/test`, { method: 'POST' });
      const data = await res.json();
      setTestResult(data);
    } catch (err) {
  ...
  const handleDelete = async (name: string) => {
    ...
    try {
      await fetch(`${API_BASE}/api/stimuli/sources/${name}`, { method: 'DELETE' });
      fetchSources();
      if (activeSource === name) {
  ...
  const getIcon = (source: StimuliSource) => {
  ...
  const saveConfig = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${API_BASE}/api/stimuli/sources/${activeSource}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        ...
        <div className="source-detail">
          {!activeSource ? (
            <div className="no-selection">
              ...
              <p>Choose a stimuli source to view its configuration, or add a new one.</p>
