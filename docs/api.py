...
# The docs directory is this module's directory
...
def build_node(dir_path: Path, rel_prefix: str = "") -> Dict[str, Any]:
    ...
    node = {
        "name": dir_path.name if rel_prefix else "docs",
        "path": rel_prefix,
        "is_folder": True,
        "children": []
    }
    try:
        for item in sorted(dir_path.iterdir()):
            # Skip hidden, pycache, and python files
            if item.name.startswith((".", "_", "__")) or item.suffix == ".py":
                continue
