...
├── stimuli/
│   ├── comms/            # External interfaces (Matrix, email)
...
| AI/ML Agent | Context Selection (φ) | Relevance scoring, attention |
| Frontend Agent | Output Layer | User-visible state |
| Vision Agent | Metacognition | Self-assessment, priority |

This is not coincidence—**the architecture that works for organizing development is the same architecture that works for organizing cognition**.
...
This recursive validation suggests the principle is general—applicable to:
- AI system design
- Team organization  
