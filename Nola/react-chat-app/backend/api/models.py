...
# Cloud models use "-cloud" suffix in ollama but are still accessed via ollama.chat()
DEFAULT_MODELS = [
    ...
    # Cloud models (accessed via ollama with -cloud tag)
...
def get_available_models() -> List[ModelInfo]:
    ...
    try:
        ...
        for m in response.get("models", []):
            ...
            display_name = model_name.replace("-cloud", " (cloud)").replace(":", " ")
        ...
        # Ollama not available, return defaults
...
async def set_current_model(request: SetModelRequest):
    ...
    # Update environment variable so agent.py picks it up
    os.environ["NOLA_MODEL_NAME"] = request.model_id
    
    # All models go through ollama (including cloud models with -cloud suffix)
