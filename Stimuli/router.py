"""
Stimuli Router - Universal API Adapter Layer

Config-driven integration with external APIs.
Add new stimuli sources by dropping a YAML file in sources/.
        ...
        for yaml_file in SOURCES_DIR.glob("*.yaml"):
            try:
                ...
                if config.enabled:
                    ...
                    print(f"✓ Loaded stimuli source: {config.name}")
...
_router: Optional[StimuliRouter] = None

def get_router() -> StimuliRouter:
    """Get singleton router instance"""
    global _router
    if _router is None:
        _router = StimuliRouter()
    return _router
