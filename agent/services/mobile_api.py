...
class QuickChat(BaseModel):
    ...
    agent: Optional[str] = None         # "aios" (default) | "copilot"
...
async def mobile_chat(msg: QuickChat, _=Depends(_check_token)):
    ...
    If `agent="copilot"` the reply is generated by forwarding into this
    Mac's VS Code Copilot Chat (via VSCodeKeyboardProvider) and polling
    the reply-file that Copilot writes back. The resulting turn lands
    in convos/convo_turns exactly like any other chat, so the phone can
    just render /api/convos/{session}/ to mirror the conversation.
    """
    if not msg.text.strip():
        raise HTTPException(status_code=400, detail="Message cannot be empty")

    provider_override = None
    agent = (msg.agent or "").strip().lower()
    if agent == "copilot":
        provider_override = "vscode"
