...
- [x] Seed identity data (user_profile, machine_context, aios_self)
...
- [x] `agent/relevance.py` → replaced by `linking_core/scoring.py`
- [x] `agent/identity_thread/` entire directory → data in DB
- [x] `agent/idv2/idv2.py` → replaced by `schema.py`
- [x] `agent/contract.py` shim → use `subconscious/contract.py`
- [x] Old `threads/*/adapter.py` files → renamed `adapter_v2.py` to `adapter.py`
- [x] `agent/subconscious/threads/` directory → adapters now in `agent/threads/*/adapter.py`
- [x] Updated imports in `subconscious/core.py` and `subconscious/__init__.py`
- [x] Cleaned stale `__pycache__` directories

**NOTE:** `agent/identity.json` kept for now (still used by agent.py bootstrap)
...
```python
from agent.threads.schema import register_module, push_to_module
...
# agent/threads/calendar/adapter.py
...
**Files to know:**
- `agent/threads/schema.py` - The foundation
- `agent/threads/base.py` - Universal adapter base
- `agent/subconscious/orchestrator.py` - Context builder
